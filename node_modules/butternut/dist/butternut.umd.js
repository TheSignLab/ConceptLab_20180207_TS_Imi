...
var Parser = function Parser(options, input, startPos) {
  ...
  // Flags to track whether we are in a function, a generator, an   function.
  this.inFunction = this.inGenerator = this.in  = false;
...
// check '  [no LineTerminator here] function'
// - '  /*foo*/ function' is OK.
// - '  /*\n*/ function' is invalid.
pp$1.is Function = function() {
  if (this.type !== tt.name || this.options.ecmaVersion < 8 || this.value != " ")
    { return false }
...
pp$1.parseFunctionStatement = function(node, is ) {
  this.next();
  return this.parseFunction(node, true, false, is )
};

pp$1.isFunction = function() {
  return this.type === tt._function || this.is Function()
};
...
pp$1.parseFunction = function(node, isStatement, allowExpressionBody, is ) {
  this.initFunction(node);
  if (this.options.ecmaVersion >= 6 && !is )
  ...
  var oldInGen = this.inGenerator, oldIn  = this.in ,
...
pp$1.parseClass = function(node, isStatement) {
  ...
  while (!this.eat(tt.braceR)) {
    ...
    if (this$1.options.ecmaVersion >= 8 && !isGenerator && !method.computed &&
        method.key.type === "Identifier" && method.key.name === " " && this$1.type !== tt.parenL &&
    ...
    if (!method.computed) {
      var key = method.key;
      if (!isGenerator && !is  && key.type === "Identifier" && this$1.type !== tt.parenL && (key.name === "get" || key.name === "set")) {
        isGetSet = true;
      ...
      if (!method.static && (key.type === "Identifier" && key.name === "constructor" ||
        ...
        if (is ) { this$1.raise(key.start, "Constructor can't be an   method"); }
        method.kind = "constructor";
        hadConstructor = true;
      }
    }
    this$1.parseClassMethod(classBody, method, isGenerator, is );
    if (isGetSet) {
...
pp$1.parseClassMethod = function(classBody, method, isGenerator, is ) {
  method.value = this.parseMethod(isGenerator, is );
...
pp$1.parseExport = function(node, exports) {
  ...
  if (this.eat(tt._default)) { // export default ...
    ...
    var is ;
    if (this.type === tt._function || (is  = this.is Function())) {
      var fNode = this.startNode();
      this.next();
      if (is ) { this.next(); }
      node.declaration = this.parseFunction(fNode, "nullableID", false, is );
...
pp$2.toAssignable = function(node, isBinding) {
  ...
  if (this.options.ecmaVersion >= 6 && node) {
    switch (node.type) {
    case "Identifier":
      if (this.in  && node.name === "await")
        { this.raise(node.start, "Can not use 'await' as identifier inside an   function"); }
      break
...
pp$3.parseMaybeUnary = function(refDestructuringErrors, sawUnary) {
  ...
  if (this.in  && this.isContextual("await")) {
...
pp$3.parseSubscripts = function(base, startPos, startLoc, noCalls) {
  ...
  var maybe Arrow = this.options.ecmaVersion >= 8 && base.type === "Identifier" && base.name === " " &&
  ...
  for (var computed;;) {
    if ((computed = this$1.eat(tt.bracketL)) || this$1.eat(tt.dot)) {
      ...
      if (maybe Arrow && !this$1.canInsertSemicolon() && this$1.eat(tt.arrow)) {
...
pp$3.parseExprAtom = function(refDestructuringErrors) {
  ...
  case tt.name:
    ...
    if (this.options.ecmaVersion >= 8 && id.name === " " && !this.canInsertSemicolon() && this.eat(tt._function))
      { return this.parseFunction(this.startNodeAt(startPos, startLoc), false, false, true) }
    if (canBeArrow && !this.canInsertSemicolon()) {
      if (this.eat(tt.arrow))
        { return this.parseArrowExpression(this.startNodeAt(startPos, startLoc), [id], false) }
      if (this.options.ecmaVersion >= 8 && id.name === " " && this.type === tt.name) {
        id = this.parseIdent();
...
pp$3.parseObj = function(isPattern, refDestructuringErrors) {
  ...
  while (!this.eat(tt.braceR)) {
    ...
    var prop = this$1.startNode(), isGenerator, is , startPos, startLoc;
    ...
    if (!isPattern && this$1.options.ecmaVersion >= 8 && !isGenerator && !prop.computed &&
        prop.key.type === "Identifier" && prop.key.name === " " && this$1.type !== tt.parenL &&
    ...
    this$1.parsePropertyValue(prop, isPattern, isGenerator, is , startPos, startLoc, refDestructuringErrors);
...
pp$3.parsePropertyValue = function(prop, isPattern, isGenerator, is , startPos, startLoc, refDestructuringErrors) {
  if ((isGenerator || is ) && this.type === tt.colon)
    { this.unexpected(); }
  ...
  if (this.eat(tt.colon)) {
    ...
    prop.kind = "init";
    prop.method = true;
    prop.value = this.parseMethod(isGenerator, is );
  } else if (this.options.ecmaVersion >= 5 && !prop.computed && prop.key.type === "Identifier" &&
             (prop.key.name === "get" || prop.key.name === "set") &&
             (this.type != tt.comma && this.type != tt.braceR)) {
    if (isGenerator || is  || isPattern) { this.unexpected(); }
...
pp$3.parseMethod = function(isGenerator, is ) {
  var node = this.startNode(), oldInGen = this.inGenerator, oldIn  = this.in ,
      oldYieldPos = this.yieldPos, oldAwaitPos = this.awaitPos, oldInFunc = this.inFunction;
...
pp$3.parseIdent = function(liberal) {
  ...
  if (this.type === tt.name) {
    ...
    if (this.in  && this.value === "await")
      { this.raiseRecoverable(this.start, "Can not use 'await' as identifier inside an   function"); }
...
var FunctionNode = (function (Node) {
	...
	// TODO `program.addWord(' ')` if necessary
...
function minifyPropertyKey ( code, chars, property, isObject ) {
	...
	if ( property.value.  || property.value.generator || property.computed || property.static || isAccessor( property ) ) {
		var prefix = '';
