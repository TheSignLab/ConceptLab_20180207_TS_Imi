...
Disallow   functions directly under `if` or `else`.
...
Fix parsing of   methods whose names are keywords.
...
Allow ` ` as a shorthand object property.
...
Don't allow the ` ` keyword to be parenthesized.
...
Properly parse properties named ` ` in ES2017 mode.
...
Support for ` `/`await` syntax when `ecmaVersion` is >= 8.
