...
NormalModuleMixin.prototype.doBuild = function doBuild(options, moduleContext, resolver, fs, callback) {
	...
	function runSyncOr (fn, context, args, callback) {
		var isSync = true;
		var isDone = false;
		...
		if(!context. ) context.  = function  () {
			if(isDone) {
				...
				throw new Error(" (): The callback was already called.");
			}
			isSync = false;
	...
	(function loadPitch() {
		...
		runSyncOr (l.module.pitch, privateLoaderContext, [remaining.join("!"), pitchedLoaders.join("!"), l.data = {}], function(err) {
	...
	function nextLoader(err/*, paramBuffer1, param2, ...*/) {
		...
		loaderContextCacheable = false;
		runSyncOr (l.module, privateLoaderContext, args, function() {
